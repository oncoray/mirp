...
def get_volumetric_morphological_features_deprecated(img_obj: ImageClass,
                                                     roi_obj: RoiClass,
                                                     settings: FeatureExtractionSettingsClass):
    """
    Extract morphological features from the image volume
    ...
    # Surface area and volume from surface mesh
    mesh_verts, mesh_faces, mesh_norms = mesh_voxels_deprecated(roi_obj=roi_obj)
...
def mesh_voxels_deprecated(roi_obj):
    """Generate a closed mesh from the morphological mask"""
    ...
    vertices, faces, norms, values = marching_cubes(
        volume=morphology_mask,
        level=0.5,
        spacing=tuple(roi_obj.roi_morphology.spacing))
