...
def get_local_intensity_features_deprecated(img_obj, roi_obj):
    """Calculate local intensity features"""
    ...
    if n_voxels > 300:
        df_local_int = compute_local_mean_intensity_filter_deprecated(img_obj=img_obj, roi_obj=roi_obj)
    elif n_voxels > 1:
        df_local_int = compute_local_mean_intensity_direct_deprecated(img_obj=img_obj, roi_obj=roi_obj)
    else:
        df_local_int = None
...
def compute_local_mean_intensity_filter_deprecated(img_obj, roi_obj):
    """Use a filter to calculate the local mean intensity"""
...
def compute_local_mean_intensity_direct_deprecated(img_obj, roi_obj):
    """Calculate mean intensity directly from the voxels"""
