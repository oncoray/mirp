...
class ContourClassDeprecated:

    def __init__(self, contour: np.ndarray, sop_instance_uid: str):
        # Convert contour to internal vertices, i.e. from (x, y, z) to (z, y, x)
        vertices = np.zeros(contour.shape, dtype=np.float64)
        vertices[:, 0] = contour[:, 2]
        vertices[:, 1] = contour[:, 1]
    ...
    def to_voxel_coordinates(self, img_obj: ImageClass):

        if self.representation == "voxel":
            return self

        self.contour = [
            np.transpose(img_obj.to_voxel_coordinates(np.transpose(contour)))
            for contour in self.contour
            if contour is not None
        ]
    ...
    def to_world_coordinates(self, img_obj: ImageClass):

        if self.representation == "world":
            return self

        self.contour = [
            np.transpose(img_obj.to_world_coordinates(np.transpose(contour)))
            for contour in self.contour
            if contour is not None
        ]
    ...
    def which_slice(self) -> List[int]:
        ...
        slice_id = np.unique(np.concatenate([
            np.rint(contour[:, 0]).astype(int)
            for contour in self.contour
            if contour is not None
        ])).tolist()
    ...
    def contour_to_grid_ray_cast(self, img_obj: ImageClass):
        from mirp.morphologyUtilities import poly2grid
