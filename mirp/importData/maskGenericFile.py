...
class MaskFile:
    ...
    def create(self):
        ...
        from mirp.importData.maskDicomFile import MaskDicomFile
        from mirp.importData.maskNiftiFile import MaskNiftiFile
        from mirp.importData.maskNrrdFile import MaskNrrdFile
        from mirp.importData.maskNumpyFile import MaskNumpyFile
    ...
    def check(self, raise_error=False):
        ...
        if not any(self.file_path.lower().endswith(ii) for ii in file_extensions):
            if raise_error:
                raise ValueError(
                    f"The file type does not correspond to a known, implemented mask type: {self.file_path}.")
        ...
        if not os.path.exists(self.file_path):
            if raise_error:
                raise FileNotFoundError(f"The mask file could not be found at the expected location: {self.file_path}")
        ...
        if self.mask_name is not None:
            if not fnmatch(os.path.basename(self.file_path), self.mask_name):
                if raise_error:
                    raise ValueError(
                        f"The file name of the mask file {self.file_path} does not match the expected pattern: "
                        f"{self.mask_name}")
