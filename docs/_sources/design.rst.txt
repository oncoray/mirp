...
first converting them to the internal format using the `ImageFile.to_object` generator method(or override methods of
subclasses), and then promoting them to the correct image modality-specific subclass using the `GenericImage.promote`
method.
